defmodule TaskManagerWeb.GroupLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.simple_form
        ...
        phx-submit="save"
      >
        <.input field={@form[:group_name]} type="text" label="Group name" />
        <:actions>
  ...
  def update(%{group: group} = assigns, socket) do
    changeset = ManageGroups.change_group(group)
  ...
  def handle_event("validate", %{"group" => group_params}, socket) do
    changeset =
      ...
      |> ManageGroups.change_group(group_params)
  ...
  defp save_group(socket, :edit, group_params) do
    case ManageGroups.update_group(socket.assigns.group, group_params) do
      {:ok, group} ->
  ...
  defp save_group(socket, :new, group_params) do
    case ManageGroups.create_group(group_params) do
      {:ok, group} ->
        notify_parent({:saved, group})
